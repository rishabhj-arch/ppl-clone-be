...
export const createPost = async (req, res: Response): Promise<void> => {
  try {
    ...
    if (!date) {
      res
        .status(400)
        .json({ success: false, message: "Please select date & time." });
      return;
    }
    if (!title) {
      res
    ...
    if (!description) {
      res
        .status(400)
        .json({ success: false, message: "Please add a description." });
      return;
    }
    if (!req.file) {
      res.status(400).json({ success: false, message: "Please add an image." });
      return;
    }
    ...
    const imageId = await uploadFileToGridFS(buffer, originalname, mimetype);
    const media = await Media.create({
      post: newPost._id,
      imageId,
      imageName: `media/${imageId.toString()}`,
      type: mimetype,
    });
...
export const editPost = async (req, res: Response): Promise<void> => {
  try {
    ...
    if ((inputDateOnly < todayDateOnly) && post.status === "SCHEDULED") {
      res.status(400).json({
        success: false,
        message: "The scheduled date must be today or in the future.",
      });
    ...
    if (req.file) {
      ...
      if (media?.imageId) {
        await deleteFileFromGridFS(media.imageId);
      }
...
export const deletePost = async (
  ...
  try {
    const { postId } = req.params;
    if (!postId) {
      res.status(400).json({ success: false, message: "PostId is require." });
      return;
    }
    const post = await Post.findById(postId);
    if (!post) {
...
export const getAllPosts = async (
  ...
  try {
    ...
    if (finalSearch && status) {
      ...
      //   { tag: { $elemMatch: { $regex: finalSearch, $options: "i" } } },
      // ];
      filter.title = { $regex: finalSearch, $options: "i" };
...
export const getPost = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.status(200).json({ success: true, message: "Post is Found.", post });
  } catch (error) {
    res
      .status(500)
      .json({ success: false, message: "Server error.", error: error.message });
